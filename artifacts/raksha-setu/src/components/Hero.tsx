  ...
  return (
    <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
      ...
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        {/* CRIME TAPE BANNER (SCROLLING MARQUEE) */}
        <div className="relative -mx-4 md:-mx-12 lg:-mx-16 mb-12 lg:mb-16 z-20 pointer-events-none overflow-hidden select-none">
          <div 
            className="bg-primary py-3 border-y-2 border-black/40 shadow-[0_4px_25px_rgba(233,79,135,0.35)] flex whitespace-nowrap overflow-hidden"
            style={{
              backgroundImage: "repeating-linear-gradient(-45deg, transparent, transparent 15px, rgba(0, 0, 0, 0.12) 15px, rgba(0, 0, 0, 0.12) 30px)",
            }}
          >
            {/* First sliding block */}
            <div className="animate-marquee flex shrink-0 items-center justify-around gap-12 min-w-full">
              {Array(6).fill("BUILDING INDIA'S NO.1 EMERGENCY SYSTEM").map((text, idx) => (
                <div key={idx} className="flex items-center gap-12 text-black font-heading font-black tracking-widest text-sm md:text-base lg:text-lg">
                  <span>{text}</span>
                  <span className="text-black/60 font-sans">•</span>
                </div>
              ))}
            </div>
            {/* Second sliding block for seamless loop */}
            <div className="animate-marquee flex shrink-0 items-center justify-around gap-12 min-w-full" aria-hidden="true">
              {Array(6).fill("BUILDING INDIA'S NO.1 EMERGENCY SYSTEM").map((text, idx) => (
                <div key={idx} className="flex items-center gap-12 text-black font-heading font-black tracking-widest text-sm md:text-base lg:text-lg">
                  <span>{text}</span>
                  <span className="text-black/60 font-sans">•</span>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-10 w-full sm:w-auto">
              <a href="https://play.google.com/store/apps/details?id=com.studioankur.rakshasetu" target="_blank" rel="noopener noreferrer">
                <Button size="lg" data-testid="button-download-now"
                  className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-white shadow-[0_0_30px_rgba(233,79,135,0.4)] hover:shadow-[0_0_50px_rgba(233,79,135,0.7)] transition-all rounded-full px-8 h-14 text-lg font-semibold border-none">
                  Download Now
                </Button>
              </a>
